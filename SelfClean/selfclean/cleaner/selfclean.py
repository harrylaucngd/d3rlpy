        ...
        if not self.cleaner.is_fitted:
            if self.model is None:
                if pretraining_type is PretrainingType.DINO:
                    ...
                    pretraining_type is PretrainingType.AE
                    or pretraining_type is PretrainingType.VAE
                ):
                    self.model = torch.load("/home/liwentao/d3rlpy/temp_result/hopper_ae_best.pt").to(f"cuda:{device}")
                else:
            ...
            if dataloader is None:
                ...
                torch_dataset = dataloader
            if pretraining_type is PretrainingType.AE:
                
                emb_space = []
                labels = None
                for batch in tqdm(torch_dataset):
                    emb_space.append(self.model.encode(batch.to(f"cuda:{device}")).cpu().detach())
                emb_space = torch.cat(emb_space, dim=0)
